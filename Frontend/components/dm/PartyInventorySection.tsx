...
import { loadItems, type Item } from "@/lib/items/item-store"
...
export function PartyInventorySection() {
  ...
  const [balanceForm, setBalanceForm] = useState({ copper: "0", silver: "0", gold: "0", platinum: "0" })
      ...
      <section className="rounded-md border border-border bg-card p-4 shadow-sm">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <label className="space-y-1 text-sm font-medium text-foreground">
            Cobre (cp)
            <Input value={balanceForm.copper} onChange={(event) => setBalanceForm((current) => ({ ...current, copper: clampMoneyValue(event.target.value) }))} inputMode="numeric" />
          </label>
          <label className="space-y-1 text-sm font-medium text-foreground">
            Plata (sp)
            <Input value={balanceForm.silver} onChange={(event) => setBalanceForm((current) => ({ ...current, silver: clampMoneyValue(event.target.value) }))} inputMode="numeric" />
          </label>
          <label className="space-y-1 text-sm font-medium text-foreground">
            Oro (gp)
            <Input value={balanceForm.gold} onChange={(event) => setBalanceForm((current) => ({ ...current, gold: clampMoneyValue(event.target.value) }))} inputMode="numeric" />
          </label>
          <label className="space-y-1 text-sm font-medium text-foreground">
            Platino (pp)
            <Input value={balanceForm.platinum} onChange={(event) => setBalanceForm((current) => ({ ...current, platinum: clampMoneyValue(event.target.value) }))} inputMode="numeric" />
          </label>
        </div>
      ...
      <section className="rounded-md border border-border bg-card p-4 shadow-sm">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(320px,420px)_1fr]">
          ...
          <div className="space-y-4">
            ...
            {isLoading ? (
              <div className="rounded-md border border-border bg-background p-4 text-sm text-muted-foreground">Cargando inventario...</div>
              ...
              filteredItems.map((item) => {
                const timestamp = formatInventoryTimestamp(item.updatedAt ?? item.createdAt)
