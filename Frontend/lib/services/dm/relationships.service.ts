import type {
  BackendDmRelationshipDto,
  BackendDmRelationshipUpsertPayload,
  DmRelationship,
  DmRelationshipDirection,
  DmRelationshipEntityType,
  DmRelationshipInput,
} from "@/lib/types"
...
function normalizeRelationship(dto: BackendDmRelationshipDto): DmRelationship {
  return {
...
function toPayload(input: DmRelationshipInput): BackendDmRelationshipUpsertPayload {
  return {
...
export async function fetchDmRelationships(): Promise<DmRelationship[]> {
  const response = await backendRequest<BackendDmRelationshipDto[]>(backendRoutes.dm.relationships)
...
export async function updateDmRelationship(id: number, input: DmRelationshipInput): Promise<DmRelationship> {
  const response = await backendRequest<BackendDmRelationshipDto>(backendRoutes.dm.relationshipById(id), {
    method: "PUT",
    body: toPayload(input),
  })
...
export async function deleteDmRelationship(id: number): Promise<void> {
  await backendRequest<void>(backendRoutes.dm.relationshipById(id), {
    method: "DELETE",
  })
}
