import type {
  BackendDmOpenLoopDto,
  BackendDmOpenLoopUpsertPayload,
  DmOpenLoop,
  DmOpenLoopInput,
  DmOpenLoopPriority,
  DmOpenLoopStatus,
...
function normalizeLoop(dto: BackendDmOpenLoopDto): DmOpenLoop | null {
...
function sanitizeInput(input: DmOpenLoopInput): BackendDmOpenLoopUpsertPayload {
  return {
...
export async function fetchDmOpenLoops(): Promise<DmOpenLoop[]> {
  const response = await backendRequest<BackendDmOpenLoopDto[]>(backendRoutes.dm.openLoops)
...
export async function updateDmOpenLoop(loopId: number, input: DmOpenLoopInput): Promise<DmOpenLoop> {
  const response = await backendRequest<BackendDmOpenLoopDto>(backendRoutes.dm.openLoopById(loopId), {
    method: "PUT",
    body: sanitizeInput(input),
  })
  const loop = normalizeLoop(response)
...
export async function deleteDmOpenLoop(loopId: number): Promise<void> {
  await backendRequest<void>(backendRoutes.dm.openLoopById(loopId), {
    method: "DELETE",
  })
}
